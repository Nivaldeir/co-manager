...
export function DateRangePicker({ value, onDateChange, mode = "range", placeholder = "Selecione um período" }: DateRangePickerProps) {
  ...
  // Detecta quando o componente chega no topo e fecha o popover se estiver aberto
  useEffect(() => {
    ...
    const checkPosition = () => {
      ...
      // Verifica se o elemento chegou no topo (com margem de 5px para evitar fechar muito cedo)
    ...
    // Adiciona listener em todos os elementos scrolláveis
    const scrollableElements: (Window | Element)[] = [window]
    
    // Encontra todos os elementos scrolláveis (incluindo o modal)
    const findScrollableParents = (element: HTMLElement | null): Element[] => {
    ...
    if (triggerRef.current) {
      const parents = findScrollableParents(triggerRef.current)
      scrollableElements.push(...parents)
    }
  ...
  return (
    <Popover open={isOpen} onOpenChange={setIsOpen}>
      ...
      <PopoverContent 
        className="w-auto p-0 max-w-[calc(100vw-2rem)] !z-[9999]" 
        align="start"
        style={{ zIndex: 9999 }}
      >
